...
// Ensures clean test vault configuration
const DIR = "/tmp/obs-e2e-closeup";
...
const driver = `(async () => {
  ...
  for (const m of MODES) {
    ...
    try {
      ...
      // Simulate switching to closeup
      try {
        view.switchToCloseup(nodeIds);
      } catch (err) { e.switchError = String(err && err.stack || err); }
      
      // Simulate switching back to panorama
      try {
        view.switchToPanorama();
      } catch (err) { e.restoreError = String(err && err.stack || err); }

    } catch (err) {
      e.switchError = "Setup error: " + String(err && err.stack || err);
    }
    out.modes.push(e);
  }
  return out;
...
const report = resp.result?.value;
if (!report) {

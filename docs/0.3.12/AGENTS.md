# AGENTS.md — Tag Lens v0.3.12 (required reading, authoritative)
...
2. **`src/view.ts` is a ~4800-line god-file.** Line numbers drift constantly; re-anchor with `grep -n` before editing.
   The split is in `0.3.12/refactor-view-split.md` (Tier 1–3 done, Tier 4 = drawing, deferred).
...
- Zero-dependency CDP driver (Node 22 global WebSocket/fetch). Reference: `test/e2e-display.mjs`.
...
## 5. References
- Design: `docs/0.3.12/basic-design.md`, `docs/0.3.12/detailed-design.md`
- In progress: `docs/0.3.12/refactor-view-split.md` (view.ts split)

...
- **Note navigator**: folder/tag trees, search, show/hide toggles, pin to sidebar.
...
| Interaction | `hit-test.ts`, `marquee-controller.ts`, `highlight.ts`, `spreadsheet-pan.ts` |
...
| Features | `freshness.ts`, `status-overlay.ts`, `tag-classification.ts`, `gap-finder.ts`, `bridge-finder.ts`, `lens-presets.ts`, `display-applicability.ts`, `image-export.ts`, `node-display.ts`, `card-sizing.ts` |
| Geometry | `aggregate-snap.ts`, `aggregate-util.ts`, `cell-snap.ts`, `conformal.ts`, `helly-detect.ts`, `subgroup-packing.ts`, `subgroup-relax.ts`, `zone-decomp.ts`, `phase-g-place.ts`, `cluster-relations.ts` |
